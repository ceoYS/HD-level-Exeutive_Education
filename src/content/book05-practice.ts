...
export const missions: MissionData[] = [
  ...
  {
    number: '05',
    title: 'Project Instructions와 Checkpoint로 긴 작업 이어가기',
    goal: '프로젝트에서 계속 지켜야 할 규칙과 지금 어디까지 왔는지를 분리해 기록하고 다음 세션으로 이어간다.',
    minutes: '5분',
    steps: [
      '프로젝트 전체에서 계속 지킬 목적·규칙·금지사항은 CLAUDE.md · AGENTS.md 같은 Project Instructions에 기록한다.',
      'Checkpoint에는 현재 목표와 이번 작업 범위를 한 줄로 남긴다.',
      '완료한 작업·남은 작업·중요한 결정 사항을 정리한다.',
      '현재 Build/Test 상태와 알려진 문제를 기록한다.',
      '다음 Agent가 먼저 읽어야 할 Project Instructions와 파일, 다음 작업을 적는다.',
      '새 세션에서는 Project Instructions와 최신 Checkpoint를 먼저 읽고 작업하게 한다.',
    ],
    prompt: {
      label: '작업을 인계할 때',
      text: '프로젝트 전체에서 계속 지켜야 할 규칙은 기존 Project Instructions(CLAUDE.md · AGENTS.md 등)를 기준으로 유지해. 지금까지의 진행 상태는 다음 세션이 바로 이어갈 수 있는 Checkpoint로 정리해줘. Goal / Scope / Completed / Decisions / Current Build-Test Status / Known Issues / Next Tasks / Files to Read 순서로 작성하고, 아직 확인하지 않은 것은 확인했다고 쓰지 마.',
      tone: 'dark',
    },
    cert: { id: 'b05-m05', statement: '나는 계속 지킬 Project Instructions와 현재 진행 상태를 담는 Checkpoint를 구분해 긴 AI 개발 작업을 이어갈 수 있다.' },
    note: 'Project Instructions는 프로젝트 전체에 계속 적용되는 규칙이고, Checkpoint는 특정 시점의 진행 상태와 다음 작업을 기록하는 문서입니다.',
  },
  {
    number: '06',
